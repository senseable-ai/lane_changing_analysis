...
def distortion_factors():
    ...
    # From the provided calibration images, 11x8 corners are identified 
    nx = 11 # 11x8 chessboard
    ny = 8
    objpoints = []
    imgpoints = []
...
def warp(img, mtx, dist): # mts, dist
    ...
    # ])
    #NOR_20230612_093659_FT.mp4
    # src = np.float32([
    #     (395, 564), #bottom-left corner
    #     (592, 407), #top-left corner
    #     (639, 407), #top-right corner
    #     (786, 720), #bottom-right corner
    # ])
    # src = np.float32([
    #     (666, 890), #bottom-left corner
    #     (920, 665), #top-left corner
    #     (1265, 890), #top-right corner
    #     (1010, 665),#bottom-right corner
    #  ]) 
    #birghtness_100
    # src = np.float32([
    #     (750, 790), #bottom-left corner
    #     (930, 645), #top-left corner
    #     (990, 645), #top-right corner
    #     (1160, 790),#bottom-right corner
    #  ]) 
    src = np.float32([
        (700, 895), #bottom-left corner
        (800, 700), #top-left corner
        (1100, 700), #top-right corner
        (1300, 895),#bottom-right corner
     ]) 
    #brightness_-100
    # src = np.float32([
    #     (750, 790), #bottom-left corner
    ...
    #원본
    # src = np.float32([
    #     (750, 810), #bottom-left corner
    #     (930, 710), #top-left corner
    #     (990, 710), #top-right corner
    #     (1160, 810),#bottom-right corner
    #  ]) 
    # src = np.float32([
    #     (750, 895), #bottom-left corner
    #     (930, 680), #top-left corner
    #     (990, 680), #top-right corner
    #     (1160, 895),#bottom-right corner
    #  ]) 
    # src = np.float32([
    #     (640, 790), #bottom-left corner
    #     (890, 645), #top-left corner
    #     (1000, 645), #top-right corner
    #     (1210, 790),#bottom-right corner
    #  ]) 
    # src = np.float32([
...
def draw_poly_lines(binary_warped, left_fitx, right_fitx, ploty):     
    ...
    result = cv2.addWeighted(out_img, 1, window_img, 0.5, 0) #(0.3)
...
def main():
    ...
    cap = cv2.VideoCapture('C:\\Users\\user\\Desktop\\im_jg_DeepSort_Pytorch_lane_detection\\yolov5\data\\video\\NOR_20230612_093659_FT.mp4') # test_sample.mp4
    if not cap.isOpened():
    ...
    out = cv2.VideoWriter('./yolov5/data/output/LD_output_lane.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
    ...
    while True:
        ...
        cv2.convertScaleAbs(frame, frame, alpha=1.0, beta=brightness_factor)
