...
import cv2
import torch
# Placeholder for previous distance
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                # Displaying the estimated speed next to the bounding box
                for *xyxy, conf, cls in reversed(det):
                    if 'speed' in locals() and speed is not None:
                        speed_text = f"Speed: {speed * 3.6:.2f} km/h"
                        cv2.putText(im0, speed_text, (int(xyxy[0]), int(xyxy[1] - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2)
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        distance = round(((h - bboxes[3]) * ratio) * 2, 1)
                        global previous_distance
                        if previous_distance is not None:
                            delta_distance = distance - previous_distance
                            # Assuming video frame rate is 30 FPS, so time between frames is 1/30 seconds
                            speed = delta_distance * 30  # speed in m/s
                            print(f"Estimated Speed: {speed * 3.6} km/h")  # converting m/s to km/h
                        previous_distance = distance
                        # Using the existing distance formula
